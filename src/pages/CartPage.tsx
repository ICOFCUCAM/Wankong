      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map(item => (
              <div key={item.id} className="flex gap-4 bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
                {item.image && <img src={item.image} alt={item.title} className="w-20 h-20 rounded-lg object-cover flex-shrink-0" />}
                <div className="flex-1 min-w-0">
                  <h3 className="text-white font-medium truncate">{item.title}</h3>
                  {item.variant && <p className="text-sm text-white/55">{item.variant}</p>}
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 bg-white/10 hover:bg-white/15 text-white rounded-lg flex items-center justify-center transition-colors">−</button>
            ...
            <button onClick={clearCart} className="text-sm text-white/55 hover:text-red-400 transition-colors">Clear cart</button>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 h-fit">
            ...
            <div className="space-y-2 text-sm mb-4">
              <div className="flex justify-between text-white/55"><span>Subtotal</span><span>${cartTotal.toFixed(2)}</span></div>
              <div className="border-t border-white/10 pt-2 flex justify-between text-white font-bold"><span>Total</span><span>${cartTotal.toFixed(2)}</span></div>
            </div>
            ...
            <Link to="/" className="block text-center text-sm text-white/55 hover:text-white mt-3 transition-colors">Continue Shopping</Link>
          </div>
        </div>
      </div>
    </div>
