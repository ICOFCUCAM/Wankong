...
- **Phase 4 — Creator storefronts** *(MVP shipped)*: persistent per-partner shoppable shop pages over the unified catalog. **Live (migration 057):** public `/s/:code` branded storefront (avatar, tagline, bio, accent) that drops the partner's 30-day attribution cookie on landing (so it *is* their tracked link), curated products via `partner_storefront_items` + `storefront_products()`, and a dashboard editor (tagline/bio/accent + add/remove featured products). **Remaining:** curated multi-store collections, brand invitations, **F6 brand→creator campaign distribution**, AI-assembled storefronts, and white-label/custom-domain portals.
...
- **Phase 9 — Agentic-commerce protocols (north star)** *(discovery half shipped)*: **F12** consume/expose **ACP (OpenAI/Stripe)** and **UCP (Google)** so AI agents transact *through* SmartKong; **F15** complete purchases inside SmartKong's AI assistant. This is what makes SmartKong "the shopping layer" in an agent-driven web — highest ceiling. **Groundwork live:** `GET/POST /api/agent-search` (machine-facing product+offer discovery, Commerce Score routed, commission never exposed), `GET /api/agent-manifest` (capability descriptor), and `POST /api/agent-checkout` (**gated MVP** behind `AGENT_API_KEY`: first-party items → hosted Stripe checkout with webhook fulfillment + partner attribution; external items → honest handoff). Full design in `docs/AGENT_CHECKOUT_DESIGN.md`. Remaining (deferred, by design): delegated one-shot payment, native ACP/UCP message formats, agent order-status/cancel API.
